    ...
    for class_name_idx = 1:numel(class_names)
        ...
        for sub_num_idx = 1:numel(sub_nums)
            ...
            if sum(size(stim_pooled_electrodes)) == 0
                stim_pooled_electrodes = zeros([height(class_labels), 1]); 
            elseif sum(size(onset_pooled_electrodes)) == 0
                onset_pooled_electrodes = zeros([height(class_labels), 1]); 
            end
            
            fs = size(stim_pooled_electrodes)
            fo = size(onset_pooled_electrodes)
            c = size(class_labels)
            oh = size(class_labels_ohe)
