...
%
%%% updated 2021/8/31 by Andrew Meier

function plot_greenlee_electrodes_on_brain(edb_nonan, electrode_table_indices, color_values, alpha, shapes, ops)
...
mni_coords(:,4) = mat2cell(ones(size(electrode_table_indices)), ones(size(electrode_table_indices, 1), 1), ones(1, size(electrode_table_indices, 2)));
mni_coords(:,5) = mat2cell(shapes, ones(size(shapes, 1), 1), ones(1, size(shapes, 2)));
mni_coords{strcmp(mni_coords(:,5), 'c') | strcmp(mni_coords(:,5), 'r'), 4} = 5; 
...
surf_show('SURFACE_ADD', ops.tempfile, 'SURFACE_PROPERTIES','color',rgbvals, 'RESOLUTION', surfshow_resolution); % colored electrode locations
