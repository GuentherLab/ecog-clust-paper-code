...
ret1=its_NUEbin(Y,jj,candidates1,c,num_rnd,minshift,alpha_rnd,quantize);
VL1=ret1.VL; % embedding vector

% Information Storage, New Information (Univariate)
out=its_SEbin(Y,VL1,jj,c,quantize);
Sy=out.Sy;
Ny=out.Hy_y;
...
ret2=its_NUEbin(Y,jj,candidates2,c,num_rnd,minshift,alpha_rnd,quantize);
VL2=ret2.VL; % embedding vector

% Information Transfer, new Information (Bivariate)
out=its_BTEbin(Y,VL2,ii,jj,c,quantize);
Txy=out.Txy;
Ny_x=out.Hy_xy;
...
ret3=its_NUEbin(Y,jj,candidates3,c,num_rnd,minshift,alpha_rnd,quantize);
VL3=ret3.VL; % embedding vector

% Conditional Information Transfer X->Y|Z, New Information (Multivariate)
out=its_PTEbin(Y,VL3,ii,jj,c,quantize);
Txy_z=out.Txy_z;
Ny_xz=out.Hy_xyz;

% Joint Information Transfer XZ->Y
out=its_BTEbin(Y,VL3,[ii kk],jj,c,quantize);
Txzy=out.Txy;
