...
% NEAREST NEIGHBOR estimation approach
clear;close all;clc;
...
ret1=its_NUEknn(Y,jj,candidates1,k,num_rnd,minshift,alpha_rnd);
VL1=ret1.VL; % embedding vector

% Information Storage, Unexplained Information (Univariate)
out=its_SEknn(Y,VL1,jj,k);
Sy=out.Sy;
Ny=out.Hy_y;
...
ret2=its_NUEknn(Y,jj,candidates2,k,num_rnd,minshift,alpha_rnd);
VL2=ret2.VL; % embedding vector

% Information Transfer, New Information (Bivariate)
out=its_BTEknn(Y,VL2,ii,jj,k);
Txy=out.Txy;
Ny_x=out.Hy_xy;
...
ret3=its_NUEknn(Y,jj,candidates3,k,num_rnd,minshift,alpha_rnd);
VL3=ret3.VL; % embedding vector

% Conditional Information Transfer X->Y|Z, Nre Information (Multivariate)
out=its_PTEknn(Y,VL3,ii,jj,k);
Txy_z=out.Txy_z;
Ny_xz=out.Hy_xyz;

% Conditional Information Transfer Z->Y|X
out=its_PTEknn(Y,VL3,kk,jj,k);
Tzy_x=out.Txy_z;

% Joint Information Transfer XZ->Y
out=its_BTEknn(Y,VL3,[ii kk],jj,k);
Txzy=out.Txy;
...
clc;
disp(['NEAREST NEIGHBOR ESTIMATOR']);
